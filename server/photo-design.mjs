  ...
  await jobs.start(async id => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (typeof encoded !== 'string' || encoded.length > MAX_BODY || !/^[A-Za-z0-9+/]+={0,2}$/.test(encoded)) throw new Error('Invalid provider image.');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  await app.listen({ port: Number(process.env.PORT || 4174), host: process.env.HOST || '127.0.0.1' });
  console.log('Photo server ready. Secrets remain on the server.');
